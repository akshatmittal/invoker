...
Treat the complete caller definition as a trust boundary and parse it with Zod before network access. Use Octokit's generated types for authentication, installation, and Workflow responses. Keep runtime checks for the Workflow's `active` state and for the newer Dispatch response fields consumed by logging: a valid Run ID and web URL. Malformed Dispatch responses are operational failures.

Startup is all-or-nothing. Before creating timers, resolve every configured repository installation, mint a repository-scoped token, and prove every target GitHub Actions Workflow exists and is active. The first installation lookup authenticates the App, so there is no separate authentication probe. With Actions-only permission the module cannot prove ref existence, the presence of `workflow_dispatch`, or the target input schema, so GitHub validates those when the Dispatch becomes due.
